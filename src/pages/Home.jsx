...
const Home = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    dispatch(getProductsByName(e.target.inputSearch.value.trim().toLowerCase(), false)) 
  }
    ...
    <div className='home'>
      ...
      <aside className='aside'>
        <div className='aside__price-container'>
          ...
          <form className={`${hiddePrice ? 'aside__form-filter' : 'aside__form-filter-hidde'}`} onSubmit={handleSubmit}>
            {/* aside__form-filter */}
            <div className='aside__input-from-container'>
              <label className='aside__input-from-label' htmlFor="from">From</label>
              <input className='aside__input-from' type="number" id='from' />
            </div>
            <div className='aside__input-to-container'>
              <label className='aside__input-to-label' htmlFor="to">To</label>
        ...
        <article className='category__container'>
          <header className='category__header'>
            ...
            <i onClick={handdleHiddeCategory} className='bx bx-chevron-down'></i>
          </header>
          <ul className={`${hiddeCategory ? 'category__list' : 'category__list-hidde'}`}>
            <li className='category__element' onClick={() => {
              dispatch(getAllProductsThunk())

              }} >All Products</li>
            {
              categories?.map(category => (
                <li className='category__element' key={category.id} onClick={() => handleClickCategory(category.id)}>{category.name}</li>
              ))
            }
          </ul>
        </article>
